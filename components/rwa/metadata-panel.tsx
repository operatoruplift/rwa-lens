...
import { z } from 'zod';
import { metadataBodySchema } from '@/lib/rwa/schema';
...
export function MetadataPanel({ identity, uri }: { identity: Identity; uri: string }) {
  ...
  const declared = identity.metadata?.uri === uri ? identity.metadata.uriFetch : undefined;
  const policy = declaredUriPolicy(uri, declared);
  async function load() {
    if (policy !== 'fetchable') return;
    setLoading(true);
    try {
  ...
  return <div className="metadata-panel"><h3>Issuer metadata</h3><p className="tabular metadata-uri">{uri}</p><p>Declared by the mint · fetch status: {outcome?.state ?? declared ?? 'skipped'}. Issuer-supplied content, not proof of backing.</p>{policy === 'fetchable' ? <button type="button" className="text-link" disabled={loading} onClick={() => void load()}>{loading ? 'Fetching metadata…' : 'Fetch metadata'}</button> : <div className="mt-3"><Callout tone="slate">{policyCopy[policy]}</Callout></div>}<p>Only deployment-allowlisted hosts may be fetched. No URI is requested automatically.</p>{outcome ? <div className="mt-3">{outcome.state === 'ok' ? <><dl>{Object.entries(outcome.body).map(([key, value]) => <div className="data-field" key={key}><dt>{key}</dt><dd>{String(value)}</dd></div>)}</dl><p className="detail-note">Fetched {outcome.fetchedAt} · {outcome.bytes} bytes.</p></> : <Callout tone={outcome.state === 'not-configured' ? 'slate' : 'amber'}><strong>{outcome.state}: </strong>{outcome.reason}</Callout>}</div> : null}</div>;
}
