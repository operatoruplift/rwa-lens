...
export function RwaLensShell({ cluster, reportsEnabled, fixturesEnabled = false, initialMint }: { cluster: Cluster; reportsEnabled: boolean; fixturesEnabled?: boolean; initialMint?: string }) {
  ...
  const [fixtureId, setFixtureId] = useState<FixtureId | null>(fixturesEnabled ? FIXTURES[0].id : null);
  ...
  const cancelInspection = useCallback(() => { requestCounter.current++; activeController.current?.abort(); }, []);

  useEffect(() => {
    const timer = !fixturesEnabled && defaultMint ? setTimeout(() => void run({ mode: 'live', cluster, mint: defaultMint }), 0) : undefined;
    return () => { clearTimeout(timer); cancelInspection(); };
  }, [cluster, defaultMint, fixturesEnabled, run, cancelInspection]);
  ...
  const exportReceipt = async (format: 'json' | 'csv') => {
    if (!result || !activeRequest || loading) return;
    setExportError(null);
    try {
      const payload = JSON.stringify(result);
      const digest = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(payload));
      const hash = Array.from(new Uint8Array(digest), byte => byte.toString(16).padStart(2, '0')).join('');
      const filename = `rwa-lens-${isFixture ? 'test-fixture' : result.identity?.mint ?? 'observation'}`;
  ...
  return (
    <div className="site-width inspection-shell">
      <section id="inspect" tabIndex={-1} className="search-panel" aria-label="Inspect a Solana token">
        ...
        <div className="example-bar"><div className="live-example"><span className="micro-label">ON SOLANA</span><button className="example-button live-example-button" type="button" disabled={loading} onClick={() => { const asset = liveAssets[0]; setMint(asset.mint); setOwner(''); if (cluster === asset.cluster) void run({ mode: 'live', cluster, mint: asset.mint }); else setNotice('USDY is on Solana mainnet. This deployment is configured for devnet; use a devnet mint on this deployment.'); }}>{liveAssets[0].symbol}<ArrowUpRight size={12} /></button><span className="example-description">Treasury-linked note</span></div>{fixturesEnabled ? <div className="fixture-examples"><span className="micro-label"><FlaskConical size={12} />SYNTHETIC</span>{FIXTURES.map(fixture => <button key={fixture.id} className={`example-button ${isFixture && fixtureId === fixture.id ? 'selected' : ''}`} type="button" aria-pressed={isFixture && fixtureId === fixture.id} disabled={loading} onClick={() => inspectFixture(fixture.id)}>{fixtureLabels[fixture.id] ?? fixture.label}</button>)}</div> : null}</div>
      ...
      {!result ? <section className="empty-balance inspection-results" aria-label="Inspection results" aria-busy={loading}><span className="empty-icon">{loading ? <LoaderCircle size={25} className="spinner" /> : <Search size={25} />}</span><p className="micro-label">{cluster === 'mainnet-beta' ? 'SOLANA MAINNET' : 'SOLANA DEVNET'}</p><h2>{loading ? 'Reading the chain…' : 'Ready for a closer look.'}</h2><p>{loading ? 'Fetching the mint account and source evidence from the configured RPC provider.' : 'Inspect a mint address to see its identity, token controls and source evidence.'}</p><noscript>Enable JavaScript to run a token inspection and export its observation.</noscript></section> : <section className={`inspection-results ${loading ? 'is-loading' : ''}`} aria-busy={loading} aria-label="Inspection results">
        ...
        <div id="inspection-identity" tabIndex={-1} className="primary-results">
          ...
          <div id="inspection-balance" tabIndex={-1} className="balance-column">{result.balances ? <BalanceCard balances={result.balances} decimals={result.identity?.decimals} synthetic={isFixture} timeline={isFixture && fixtureId === 'treasury-scaled' ? <div className="timeline"><div className="timeline-top"><span><FlaskConical size={13} />SCHEDULED MULTIPLIER TEST</span><span className="raw-fixed">RAW UNITS STAY FIXED</span></div><div className="timeline-track" role="group" aria-label="Scheduled multiplier timeline">{(['before', 'at', 'after'] as const).map(side => <button key={side} type="button" disabled={loading} aria-pressed={scenario === side} onClick={() => inspectFixture('treasury-scaled', side)}><span className="timeline-point" /><span>{scenarioLabels[side]}</span><strong>{side === 'before' ? '× 1.04235' : '× 1.05114'}</strong></button>)}</div><p>Changes the test observation time. No tokens move.</p></div> : undefined} /> : <div className="empty-balance"><span className="empty-icon"><Search size={25} /></span><p className="micro-label">HOLDER BALANCE</p><h2>{result.balanceStatus === 'unavailable' ? 'Balance unavailable' : 'Add a wallet. See the full picture.'}</h2><p>{result.balanceStatus === 'unavailable' ? 'The public account lookup did not complete. The amount is unknown. Retry with the same wallet when the provider is available.' : 'Mint identity is available above. Add a public wallet address to reconcile its exact raw units and displayed amount.'}</p><button type="button" className="text-link" onClick={() => document.getElementById('rwa-owner')?.focus()}>Enter a wallet address <ArrowRight size={15} /></button><span>No signature required</span></div>}</div>
        </div>
